...
{ 
  lagged.df <- lagged.df %>%
    select(-rougher_input_state_columns)
}
...
predictions <- h2o.predict(aml.final, test_data %>% select(-date) %>% as.h2o()) %>% as.data.frame()
results <- ifelse(predictions$predict < 0, 0, ifelse(predictions$predict > 100, 100, predictions$predict))
results %>% hist(col = "red2")
test_set <- test_set %>% mutate(final.output.recovery = results) %>% select(date, final.output.recovery)
write_rds(test_set, "./submissions/feb-17/test_set_final_recovery.rds")
