...
data_final_NN <- lagged.df %>%
  mutate(final_recovery_predictions = ifelse(predictions < 0, 0, predictions)) %>% 
  mutate(residuals = get(output_cols) - final_recovery_predictions, 
         label = ifelse(row_number() %in% (train_idx %>% sort()), "train", "test"))

MAE <- data_final_NN %>% filter(label == "test") %>% pull(residuals) %>% abs() %>% sum()/nrow(data_final_NN)
MAE*std[output_cols]
...
ggplot(data_final_NN %>% mutate(year = lubridate::year(date)) %>% filter(year >= lubridate::year("2016-01-01"))) +
  geom_point(aes(x = date, y = get(output_cols)), color = "blue2") +
  geom_line(aes(x = date, y = get(output_cols)), color = "blue2") +
...
ggplot(data_final_NN) + 
  geom_point(aes(x = get(output_cols), y = final_recovery_predictions, color = label))
